...
public class DetectorYolo5 : MonoBehaviour, Detector
{
    ...
    private async void AsyncParseOutputs(Tensor yoloModelOutput, float threshold,Dictionary<string, float> scores){
        await Task.Run(() => ParseOutputs(yoloModelOutput, MINIMUM_CONFIDENCE, scores));
    }
