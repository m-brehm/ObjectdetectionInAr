...
//    }
    ...
    private ArrayList<Recognition> getDetectionsForTiny(ByteBuffer byteBuffer, Bitmap bitmap) {
        ...
        for (int i = 0; i < gridWidth;i++){
            ...
            final float score = maxClass;
            if (score > getObjThresh()){
                //LOGGER.w(bboxes[0][i][0]+" "+bboxes[0][i][1]+" "+bboxes[0][i][2]+" "+bboxes[0][i][3]+" "+detectedClass);
                final float xPos = bboxes[0][i][0];
                final float yPos = bboxes[0][i][1];
                final float w = bboxes[0][i][2];
                final float h = bboxes[0][i][3];
...
//        }
    ...
    public ByteBuffer preProcess(Bitmap bitmap){
        bitmap = Utils.processBitmap(bitmap, 416);
        ByteBuffer byteBuffer = convertBitmapToByteBuffer(bitmap);
        return byteBuffer;
    }

    public Map<Integer, Object> runInference(ByteBuffer byteBuffer){
        Map<Integer, Object> outputMap = new HashMap<>();
        outputMap.put(0, new float[1][OUTPUT_WIDTH_TINY[0]][4]);
        outputMap.put(1, new float[1][OUTPUT_WIDTH_TINY[1]][labels.size()]);
        Object[] inputArray = {byteBuffer};
    ...
    public List<Recognition> postProcess(Map<Integer, Object> modelOutput, Bitmap bitmap){
        ArrayList<Recognition> detections = new ArrayList<Recognition>();
        int gridWidth = OUTPUT_WIDTH_TINY[0];
        float[][][] bboxes = (float [][][]) modelOutput.get(0);
        float[][][] out_score = (float[][][]) modelOutput.get(1);
